...
export class ArticleController {
    ...
    @Patch('update')
    async updateArticle(@Req() request: Request ,@Body() articleUpdate : articleUpdateBodyDto){
        const userJwt = request['user']

        const article = await this.articleService.getArticleById(articleUpdate.id_post)
        ...
        if(article.id_user_who_post.id_user !== userJwt.userId ){
            throw new Error('Vous ne pouvez pas mettre à jour un article qui n\'est pas le votre !')
        }
